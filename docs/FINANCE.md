...
1. **Apply the schema.** Open the Supabase SQL Editor and run
   ...
   Idempotent — re-running is safe. Then run
   [`docs/supabase/invoices-schema.sql`](supabase/invoices-schema.sql) the same
   way — it adds the `clients` and `invoices` tables (and seeds Superlogics AS).
...
| **Finance → Invoices** | Receivables (`docs/supabase/invoices-schema.sql`). Numbering continues from the old tool: the system suggests **INV-00217** onwards, editable before saving, unique forever. An invoice lives through **draft → sent → paid**; overdue is derived (sent + past due), never stored. The form picks a customer from the Customers tab (address, currency prefill), computes the due date from Net-N terms, takes line items (qty × rate), and prints bank details in the notes by default. PDF on the real letterhead mirrors the old INV-00216 layout. **Record payment** asks for the PKR that actually landed after remittance and posts ONE income entry to the ledger (category = the customer's income source, description `Invoice INV-NNNNN — Customer`); deleting that ledger entry later reverts the invoice to *sent*. Deleting a paid invoice removes its income entry too, after an explicit warning. |
| **Finance → Customers** | The customer book — add a customer here first, then pick them on the invoice form. Name, billing address (printed on invoices), invoice currency (NOK, USD, PKR…), and the ledger income source their payments post to. A customer with invoices cannot be deleted (app message + `ON DELETE RESTRICT`); mark them inactive instead. Each card shows invoice count and what they still owe in their currency. |
| **Finance → Reports** | The report builder: any period (presets for **FBR fiscal years, 1 Jul–30 Jun**, calendar years, this/last month, or custom dates) × any slice (income only / expenses only / one category), with live totals including the period's opening and closing balance. Downloads: **branded P&L PDF on the letterhead**, **balance sheet PDF** (cash basis, as of any date), **general ledger CSV** (per-account running balances), **trial balance CSV**, report CSV, transactions CSV, and fiscal/calendar/monthly closing tables. |
